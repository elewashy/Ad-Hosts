...
(function () {
  ...
  // =========================================================
  // BYPASS LAYER — Section 9: Periodic Guards & Cleanup
  // Targets: rm.freex2line.online | cimanow.cc
  // Runs at onPageFinished — DOM is ready, call cleanup directly.
  // =========================================================
  if (
    hostname.includes('freex2line.online') ||
    hostname.includes('cimanow.cc')
  ) {
    (function () {
      // Elements to restore if an anti-adblock script hid them
      var UNHIDE = ['.watch__area', '.downloads__tabs', '#player', '.player-container'];

      // Known anti-adblock notice selectors to forcibly remove
      var BLOCK_NOTICES = [
        '.anti-adblock-message', '#anti-adblock', '#adblock-notice',
        '#adblock-overlay', '.adblock-overlay', '.adblock-wall',
        '#anti-adb-overlay', '.swal2-container',
      ];

      // Keep a direct reference to the native remove so our DOM-protection
      // wrapper (installed in script_2.js Section 8) doesn't interfere here.
      var _nativeRemove = Element.prototype.remove;

      function periodicCleanup() {
        // 1. Remove any injected adblock-notice overlays
        BLOCK_NOTICES.forEach(function (sel) {
          document.querySelectorAll(sel).forEach(function (el) {
            try { _nativeRemove.call(el); } catch (e) {}
          });
        });

        // 2. Un-hide content sections the detection script may have hidden
        UNHIDE.forEach(function (sel) {
          var el = document.querySelector(sel);
          if (el) {
            var s = el.getAttribute('style') || '';
            if (/display\s*:\s*none/i.test(s) || s.includes('visibility: hidden')) {
        ...
        // 3. Release body scroll-lock set by overlay scripts
        if (document.body) {
      ...
      // Run once immediately, then keep running for 30 s
      periodicCleanup();
      var cleanupInterval = setInterval(periodicCleanup, 800);
      setTimeout(function () { clearInterval(cleanupInterval); }, 30000);

      // ── freex2line.online specific ──
      if (hostname.includes('freex2line.online')) {
        var freexInterval = setInterval(function () {
          var btn = document.querySelector('#downloadbtn, .downloadbtn, #download-button, #d-button');
          if (btn) {
            btn.style.setProperty('display', 'inline-block', 'important');
            btn.style.setProperty('opacity', '1', 'important');
            btn.style.setProperty('visibility', 'visible', 'important');
            var loader = document.querySelector('#download-loading, #d-loading, .download-loading');
            if (loader) loader.style.display = 'none';
            clearInterval(freexInterval);
          }
        }, 500);
        setTimeout(function () { clearInterval(freexInterval); }, 30000);
      }

      // ── cimanow.cc specific ──
      if (hostname.includes('cimanow.cc')) {
        var cimaInterval = setInterval(function () {
          ['.watch__area', '.downloads__tabs', '#player', '.player-container'].forEach(function (sel) {
            var el = document.querySelector(sel);
        ...
        setTimeout(function () { clearInterval(cimaInterval); }, 30000);
      }
    })();
  }
  // ── end bypass section 9 ──
