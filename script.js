...
// ==UserScript==
// @name         Base64 URL Decoder for Arabic Video Sites
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  Decodes Base64 URLs and replaces them with direct links
// @author       elewashy
// @match        *://*.gamehub.cam/*
// @match        *://*.reviewrate.net/*
// @grant        none
// ==/UserScript==
...
(function() {
    ...
    // Find all links that contain Base64 encoded URLs
    const links = document.querySelectorAll('a[href*="/l/aHR0"]');
    
    links.forEach(link => {
        const href = link.getAttribute('href');
        
        // Extract the Base64 part from the URL
        const base64Match = href.match(/\/l\/(aHR0[^/]*)/);
        
        if (base64Match && base64Match[1]) {
            const base64Url = base64Match[1];
            const decodedUrl = decodeBase64(base64Url);
            
            // Set the new direct URL
            link.setAttribute('href', decodedUrl);
            
            // Optional: Add a visual indicator that the link was decoded
            link.style.border = '2px solid green';
            
            // Log the transformation for debugging
            console.log(`Transformed: ${href} -> ${decodedUrl}`);
        }
    });
    
    const downloadBlocks = document.querySelectorAll('.DownloadBlock');
    downloadBlocks.forEach(block => {
        // Keep only the necessary links within the download servers section
        const serverLinks = block.querySelectorAll('.DownloadServers a');
        const container = block.parentNode;
        
        // Create a new clean container for the links
        const newContainer = document.createElement('div');
        newContainer.className = 'clean-download-links';
        
        serverLinks.forEach(serverLink => {
            newContainer.appendChild(serverLink.cloneNode(true));
        });
        
        // Replace the old content with just the cleaned-up links
        container.innerHTML = '';
