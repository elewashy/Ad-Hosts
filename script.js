...
// ==UserScript==
// @name         Iframe Delay Skipper
// @namespace    http://tampermonkey.net/
// @version      0.2
// @description  Accelerates delay timers in iframes
// @author       elewashy
// @match        *://*/*
// @grant        unsafeWindow
...
(function() {
    ...
    // Function to override setTimeout in the current window
    function overrideSetTimeout(win) {
        const originalSetTimeout = win.setTimeout;
        
        win.setTimeout = function(callback, delay, ...args) {
            // Check if this is the specific setTimeout we want to target
            if (delay === 5000 && callback.toString().includes('blk1') && callback.toString().includes('hide')) {
                console.log('Accelerating 5-second delay for #blk1.hide() to 50ms');
                return originalSetTimeout(callback, 50, ...args); // Reduce 5s to 50ms
            }
            
            // For any setTimeout with delay greater than 1 second
            if (delay > 1000) {
                console.log('Reducing delay from ' + delay + 'ms to 100ms');
                return originalSetTimeout(callback, 100, ...args); // Reduce all delays to 100ms
            }
            
            // For other setTimeouts, keep them as is
            return originalSetTimeout(callback, delay, ...args);
        };
    }
    
    // Override setTimeout in the main window
    overrideSetTimeout(window);
    
    // Function to process an iframe
    function processIframe(iframe) {
        try {
            // Only process if we can access the iframe's contentWindow
            if (iframe.contentWindow) {
                // Override setTimeout in this iframe
                overrideSetTimeout(iframe.contentWindow);
                
                // Also try to directly find and hide the #blk1 element
                try {
                    if (iframe.contentDocument && iframe.contentDocument.getElementById('blk1')) {
                        console.log('Found #blk1 in iframe, hiding immediately');
                        iframe.contentDocument.getElementById('blk1').style.display = 'none';
                    }
                } catch (e) {
                    // Cross-origin iframe, can't access contents directly
                }
            }
        } catch (e) {
    ...
    // Process all iframes immediately and when DOM changes
        ...
        mutations.forEach(function(mutation) {
            if (mutation.addedNodes && mutation.addedNodes.length > 0) {
                for (let i = 0; i < mutation.addedNodes.length; i++) {
                    ...
                    // If we added an element that might contain iframes
    ...
    // Additional code to override delay_done variable if it exists
    function checkAndOverrideDelayDone() {
        try {
            // Try in main window
            if (typeof window.delay_done !== 'undefined') {
                window.delay_done = 1;
                console.log('Set delay_done to 1 in main window');
            }
            ...
            iframes.forEach(iframe => {
                try {
                    if (iframe.contentWindow && typeof iframe.contentWindow.delay_done !== 'undefined') {
                        iframe.contentWindow.delay_done = 1;
                        console.log('Set delay_done to 1 in iframe');
                    }
                } catch (e) {
            ...
            console.log('Error checking delay_done variable', e);
        }
    }
    
    // Check for delay_done variable periodically
    setInterval(checkAndOverrideDelayDone, 1000);
})();
