...
// First, add required CSS
const style = document.createElement('style');
style.textContent = `
    .protected-iframe-container {
        position: relative;
        width: 100%;
        height: 100%;
    }
...
class IframeProtector {
    ...
    setupIframes() {
        ...
        Array.from(iframes).forEach(iframe => this.protectIframe(iframe));
    }

    protectIframe(iframe) {
        // 1. Wrap iframe in container
        const container = document.createElement('div');
        container.className = 'protected-iframe-container';
        iframe.parentNode.insertBefore(container, iframe);
        container.appendChild(iframe);

        // 2. Add overlay to catch all events
        const overlay = document.createElement('div');
        overlay.className = 'click-overlay';
        container.appendChild(overlay);

        // 3. Set strict sandbox attributes
        iframe.setAttribute('sandbox', 'allow-same-origin allow-scripts');
        
        // 4. Store original src
        const originalSrc = iframe.src;

        // 5. Add comprehensive protection
        this.addIframeProtection(iframe, overlay, originalSrc);
    }

    addIframeProtection(iframe, overlay, originalSrc) {
        // Capture and analyze all clicks
        overlay.addEventListener('click', (e) => {
            ...
            try {
                ...
                const clickedElement = iframeDoc.elementFromPoint(x, y);

                if (this.isNavigationAttempt(clickedElement)) {
                ...
                // Allow the click for non-navigation elements
                ...
                if (clickedElement && !this.isNavigationAttempt(clickedElement)) {
                    clickedElement.dispatchEvent(clonedEvent);
                }
            } catch (error) {
                console.log('Cross-origin iframe, applying strict protection');
            }
        }, true);

        // Override iframe methods and properties
        this.applyIframeProtections(iframe, originalSrc);
        
        // Handle cross-origin iframes
        this.handleCrossOrigin(iframe, originalSrc);
    }

    isNavigationAttempt(element) {
        ...
        // Check for other navigation triggers
        const isNavTrigger = element.getAttribute('target') === '_blank' ||
                            element.getAttribute('rel') === 'popup' ||
                            element.dataset.href ||
                            element.role === 'link';

        return isLink || isNavTrigger;
    }

    applyIframeProtections(iframe, originalSrc) {
        try {
            ...
            console.log('Cannot access iframe content methods');
        }
    }

    handleCrossOrigin(iframe, originalSrc) {
        // Monitor src changes
        ...
        // Reset src if navigation occurs
